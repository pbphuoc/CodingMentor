...
public class BookStoreService {
	...
	public String getBorrowStatus(BorrowReturnHistory borrowReturnHistory) {
		LocalDate todayDate = LocalDate.parse("24-10-2022", formatter);
	...
	public void showBorrowListByAllCustomer(ArrayList<BorrowReturnHistory> borrowReturnHistories,
		...
		String format = "%-15s %-35s %-30s %-30s %-15s %-15s %-15s\n";
		System.out.printf(format, "Customer", "Book", "Author", "Type(s)", "Borrow Date", "Return Date", "Status");
			...
			for (BorrowReturnHistory history : borrowReturnHistories) {
				if (history.getCustomerID().equalsIgnoreCase(customer.getCustomerID())) {
					...
					System.out.printf(format, customer.getCustomerName(), b.getName(), b.getAuthor(), bookTypes,
							history.getBorrowDate(), history.getActualReturnDate(), getBorrowStatus(history));
				}
			}
		}
	}
